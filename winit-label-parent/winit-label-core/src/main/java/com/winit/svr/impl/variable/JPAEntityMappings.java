...
public class JPAEntityMappings {
  ...
  public String getJPAClassString(Object value) {
    if(value == null) {
      throw new LabelIllegalArgumentException("null value cannot be saved");
    }
    
    EntityMetaData metaData = getEntityMetaData(value.getClass());
    if(!metaData.isJPAEntity()) {
      throw new LabelIllegalArgumentException("Object is not a JPA Entity: class='" + value.getClass() + "', " + value);
    }
  ...
  public Object getIdValue(Object value, EntityMetaData metaData) {
    try {
      ...
      throw new LabelException("Illegal argument exception when getting value from id method/field on JPAEntity", iae);
    } catch (IllegalAccessException iae) {
      throw new LabelException("Cannot access id method/field for JPA Entity", iae);
    } catch (InvocationTargetException ite) {
      throw new LabelException("Exception occured while getting value from id field/method on JPAEntity: " + 
        ite.getCause().getMessage(), ite.getCause());
    }
    
    // Fall trough when no method and field is set
    throw new LabelException("Cannot get id from JPA Entity, no id method/field set");
  }
  ...
  public Object getJPAEntity(String className, String idString) {
    ...
    if(metaData == null) {
      throw new LabelIllegalArgumentException("Class is not a JPA-entity: " + className);
    }
  ...
  private Object findEntity(Class< ? > entityClass, Object primaryKey) {
    ...
    if(entity == null) {
      throw new LabelException("Entity does not exist: " + entityClass.getName() + " - " + primaryKey);
    }
    return entity;
  }
  ...
  public Object createId(EntityMetaData metaData, String string) {
    ...
    if(type == Long.class || type == long.class) {
      ...
      throw new LabelIllegalArgumentException("Unsupported Primary key type for JPA-Entity: " + type.getName());
    }
  }
  
  public String getIdString(Object value) {
    if(value == null) {
      throw new LabelIllegalArgumentException("Value of primary key for JPA-Entity cannot be null");
    }
    ...
    if(value instanceof java.util.Date){
      ...
      throw new LabelIllegalArgumentException("Unsupported Primary key type for JPA-Entity: " + value.getClass().getName());
    }
  }
}
