 ...
 */
package com.winit.svr.cfg;

import com.winit.svr.impl.cfg.LabelServerConfigurationImpl;


/**
 * Implementations of this class can be plugged into a {@link LabelServerConfigurationImpl}.
...
public interface ProcessEngineConfigurator {
	 ...
	 * 
	 * Allows to tweak the process engine by passing the {@link LabelServerConfigurationImpl}
   * which allows tweaking it programmatically.
	 ...
	 * for the jdbc url.
	 */
	void beforeInit(LabelServerConfigurationImpl processEngineConfiguration);
   ...
   * No default properties need to be overridden for this (otherwise the {@link #beforeInit(LabelServerConfigurationImpl)} 
   * method should be used) so the logic contained in this method is executed
   * after initialisation of the default objects.
   * 
   * Probably a better name would be 'afterInit' (cfr {@link #beforeInit(LabelServerConfigurationImpl)}),
   * but not possible due to backwards compatibility.
   */
  void configure(LabelServerConfigurationImpl processEngineConfiguration);
