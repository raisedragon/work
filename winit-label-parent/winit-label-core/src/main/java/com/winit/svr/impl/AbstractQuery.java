...
import com.winit.svr.impl.db.ListQueryParameterObject;
...
public abstract class AbstractQuery<T extends Query<?,?>, U> extends ListQueryParameterObject implements Command<Object>, Query<T,U>, Serializable {
  ...
  public T direction(Direction direction) {
    if (orderProperty==null) {
      throw new LabelIllegalArgumentException("You should call any of the orderBy methods first before specifying a direction");
    }
  ...
  protected void checkQueryOk() {
    if (orderProperty != null) {
      throw new LabelIllegalArgumentException("Invalid query: call asc() or desc() after using orderByXX()");
    }
  }
  ...
  public U executeSingleResult(CommandContext commandContext) {
    ...
    if (results.size() == 1) {
     ...
     throw new LabelException("Query return "+results.size()+" results instead of max 1");
    } 
    return null;
  }
