...
import com.winit.svr.impl.persistence.LabelEntityManagerFactory;
import com.winit.svr.impl.persistence.LogisticTypeEntityManagerFactory;
...
import com.winit.svr.label.LabelHandlerFactory;
...
public abstract class LabelServerConfigurationImpl extends LabelServerConfiguration {  
  ...
  //TODO
  protected LabelHandlerFactory labelHandlerFactory;
  ...
  private void initLabelHandlerFactory()
{
	if(this.labelHandlerFactory==null){
		this.labelHandlerFactory = new LabelHandlerFactory();
		this.labelHandlerFactory.setHandlers(this.labelHanlers);
	}
...
protected void initCommandExecutors() {
  ...
  protected CommandInterceptor initInterceptorChain(List<CommandInterceptor> chain) {
    if (chain==null || chain.isEmpty()) {
      throw new LabelException("invalid command interceptor chain configuration: "+chain);
    }
    ...
    if (dataSource==null) {
      if (dataSourceJndiName!=null) {
        try {
          ...
          throw new LabelException("couldn't lookup datasource from "+dataSourceJndiName+": "+e.getMessage(), e);
        }
        
      } else if (jdbcUrl!=null) {
        if ( (jdbcDriver==null) || (jdbcUrl==null) || (jdbcUsername==null) ) {
          throw new LabelException("DataSource or JDBC properties have to be specified in a process engine configuration");
        }
  ...
  public void initDatabaseType() {
    ...
    try {
      ...
      if (databaseType==null) {
        throw new LabelException("couldn't deduct database type from database product name '"+databaseProductName+"'");
      }
  ...
  protected void initSqlSessionFactory() {
    if (sqlSessionFactory==null) {
      ...
      try {
        ...
        throw new LabelException("Error while building ibatis SqlSessionFactory: " + e.getMessage(), e);
      } finally {
...
//  		getEventDispatcher().addEventListener(new EventLogger(clock));
  ...
  public LabelServerConfigurationImpl setLabelService(LabelService labelService) {
  ...
  public void setEventListeners(List<ActivitiEventListener> eventListeners) {
	...
	public void setLabelHandlerFactory(LabelHandlerFactory labelHandlerFactory)
	{
		this.labelHandlerFactory = labelHandlerFactory;
	}
	
	public LabelHandlerFactory getLabelHandlerFactory(){
		return this.labelHandlerFactory;
	}
